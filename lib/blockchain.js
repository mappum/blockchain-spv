...
Blockchain.prototype.getBlockAtHeight = function (height, cb) {
  ...
  function traverse (err, block) {
    ...
    self.getBlock(down ? block.header.prevHash : block.next, traverse)
  }
...
Blockchain.prototype.createReadStream = function (opts) {
  var self = this
  opts = opts || {}
  var cursor = opts.from || this.genesis.hash
  ...
  function read (size, next) {
    ...
    if (cursor.equals(u.nullHash)) {
      // we reached end of chain, wait for new block
      var onBlock = (block) => {
        if (!block.header.prevHash.equals(lastHash)) return
        self.removeListener('block', onBlock)
        pause = false
        cursor = block.header.getHash()
        nextBlock.call(this, next)
      }
      self.on('block', onBlock)
      return
    }
  ...
  function nextBlock (next) {
    self.getBlock(cursor, (err, block) => {
      if (err) return next(err)
      cursor = block.next
      lastHash = block.header.getHash()
      pause = false
      if ((opts.stopHash && opts.stopHash.equals(lastHash)) ||
      (opts.stopHeight && opts.stopHeight === block.height)) {
        this.push(block)
