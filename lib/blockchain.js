...
var Blockchain = module.exports = function (opts) {
  ...
  this.opts = opts
  this.timestampThreshold = opts.params.timestampThreshold || 500000000

  var genesisHeader = new bitcore.BlockHeader(opts.params.genesisHeader)
  ...
  if (opts.checkpoints) {
    this.checkpoint = opts.checkpoint[opts.checkpoint.length - 1]
...
Blockchain.prototype._onHeaders = function (message) {
  ...
  this.processHeaders(message.headers, function (err, last) {
    if (err) {
      return self._error(new Error('Received invalid headers: ' + err.message))
    }
    if (self.syncing) {
      self.emit('sync', last)
      self._getHeaders()
...
Blockchain.prototype.processHeader = function (prev, header, cb) {
  ...
  if (!params.shouldRetarget(block) && header.bits !== prev.header.bits) {
  ...
  if (params.shouldRetarget(block) &&
  // don't verify retarget if it requires checking before our checkpoint
  !(this.checkpoint && height - this.checkpoint.height < params.interval)) {
    return params.calculateTarget(block, this, function (err, target) {
...
Blockchain.prototype.validProof = function (header) {
  var hash = this.opts.params.miningHash(header)
...
Blockchain.prototype.maxTarget = function () {
  ...
  return u.expandTarget(params.genesisHeader.bits)
}
